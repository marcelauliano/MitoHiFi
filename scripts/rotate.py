...
def parseArgs():
	parser = argparse.ArgumentParser(description='rotates contigs (or '
		'scaffolds) individually', add_help=False)
	...
	opt.add_argument('-r', '--rotate', type=int, default=50000, metavar='INT',
		help='rotation size (in bp); negative values turn counterclockwise '
		'from 3\' to 5\' [50,000]')
	opt.add_argument('--force', default=False, action='store_true',
		help='force rotations for all sequence records')
	opt.add_argument('--skip-failures', default=False, action='store_true',
...
def main():
	...
	if rotation_size > 0:
		sys.stderr.write('INFO: rotating each sequence record {} bp '
			'clockwise...\n\n'.format(rotation_size))
	elif rotation_size < 0:
		sys.stderr.write('INFO: rotating each sequence record {} bp '
			'counterclockwise...\n\n'.format(abs(rotation_size)))
	else:
		sys.stderr.write('ERROR: the number of base pairs to rotate each '
	...
	if any(x <= abs(rotation_size) for x in records_lengths):
		if args.force or args.skip_failures:
			sys.stderr.write('INFO: at least one input record is not longer '
				'than {} bp rotation size.\n\n'.format(abs(rotation_size)))
		else:
			sys.stderr.write('ERROR: at least one input record is not longer '
	...
	for record in SeqIO.parse(infile, 'fasta'):
		...
		if seq_length > abs(rotation_size):
			...
			record.seq = Seq(rotd_seq, Alphabet.generic_dna)
			rotated_records.append(record)
			sys.stderr.write('INFO: {} sequence ({} bp) was rotated {} bp.\n'.\
				format(record.id, seq_length, abs(rotation_size)))
		else:
			if args.force:
				spin_size = rotation_size % len(record.seq)
				rotated_seq = init_seq[spin_size:] + init_seq[:spin_size]
				record.seq = Seq(rotated_seq, Alphabet.generic_dna)
				rotated_records.append(record)
				sys.stderr.write('INFO: {} sequence ({} bp) is not longer '
					'than the specified {} bp rotation size. It was rotated '
					'{} bp.\n'.format(record.id, seq_length,
						abs(rotation_size), spin_size))
